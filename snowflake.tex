...
%     "91DA221A149007D0FD9E5515F5786C3DD07E4BB0" = "snowflake-02"
%   )
% > read_csv("figures/users/bandwidth-multi.csv") %>%
...
\caption{
Average simultaneous Snowflake users and bandwidth by day.
The values at the far left,
in early January 2021, are about~60 simultaneous users
...
%     ) %>%
%     mutate(across(c(read, write, `dirreq-read`, `dirreq-write`, good_read, good_write, good_avg), scales::label_bytes(units = "auto_si", accuracy = 0.01))) %>%
%     arrange(date) %>% filter("2020-12-29" <= date & date < "2021-01-04")
% # A tibble: 6 × 8
%   date       read     write   `dirreq-read` `dirreq-write` good_read good_write good_avg
%   <date>     <chr>    <chr>   <chr>         <chr>          <chr>     <chr>      <chr>
...
The contribution of a client is the same,
no~matter how many temporary proxies it may use
...
Network censorship events may result in fewer
or more users of a circumvention system:
fewer if the system has not enough resistance to prevent
...
had the effect of increases in the number of Snowflake users
by multiples.
