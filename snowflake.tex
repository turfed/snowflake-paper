...
A~HyperLogLog++ sketch supports two basic operations: count and merge.
Given a sketch~\(X\),
we may compute an approximate count \(|X|\)
of its unique elements;
given two sketches \(X\) and~\(Y\),
we may merge them to form a new sketch
...
It~takes about 20~hours for 50\% of the proxy pool to be replaced.
}
...
distributing traffic over multiple bridges.
