...
We condense NAT and firewall features
into the following well-known variations:\todo{
Cut these definitions and replace with a reference to RFC 3489\cite[\S 5]{rfc3489}?
}
...
Any remote host may send a packet to an internal IP--port pair through its
external port.
\item[Restricted cone]
Like full cone,
but inbound packets
are allowed only if
there has recently been an outbound packet
to the same remote IP address.
\item[Port-restricted cone]
Like restricted cone,
but inbound packets are allowed only if
there has recently been an outbound packet
to the same remote IP--port pair.
\item[Symmetric]
The external port depends on both
the internal and external IP--port pairs.
...
but the type of port-restricted cone NAT differs
...
between a port-restricted cone client and a symmetric proxy,
