...
nor even their eventual destinations.)
...
what Tschantz et~al.\ have observed:
...
only as a last resort~\cite[\mbox{VI-A}]{Tschantz2016a}.
...
\begin{description}
\item[Rendezvous]
(e.g.\ TLS fingerprinting of domain fronting or AMP cache rendezvous)
designed to be blocking-resistant on its own, but may combine with other features
\autoref{sec:block-ir}
\ldots

\item[Selection of STUN servers]
\ldots

\item[Form and content of STUN messages]
\todo{Check if ``message'' is the right terminology}
\ldots

\item[DTLS]
handshake fingerprinting
just like TLS
selection and order of ciphersuites,
selection and order of extensions
\autoref{sec:block-ru}
\ldots

\item[Data channel or media channel]
\ldots
