...
\caption{
Number of unique Snowflake proxy IP addresses per day,
by NAT type.
}
\label{fig:proxy-nat}
\todo[inline]{Explain temporary inversions of restricted and unknown.}
\end{figure}
...
% > x %>% mutate(percent = 100 * users / sum(users))
% # A tibble: 3 x 4
...
% > x %>% filter(mode == "bridge") %>% mutate(percent = 100 * users / sum(users))
% # A tibble: 2 x 4
%   mode   transport       users percent
%   <chr>  <chr>           <dbl>   <dbl>
...
it was far from certain that it would be possible to recruit
enough volunteer proxies
to be meaningfully blocking resistant
and support a respectable number of users.
\todo{Give estimated count of flash proxies from
\href{https://people.torproject.org/~dcf/graphs/}{fp-num-proxies-201601.png}.}

% "Start producing snowflakes" https://gitlab.torproject.org/tpo/anti-censorship/pluggable-transports/snowflake/20813
...
\autoref{fig:proxy-type} shows the total count of unique proxy IP addresses
...
as shown in \autoref{fig:proxy-type}.
...
\todo{Figure out timing of Orbot ``iptproxy'' self-labeling.}
% On June 2022-06-21 there is a shift from "Unknown" to "Orbot", but it doesn't
% quite add up. One thing is that the series do not quite match up at the seam:
% "Orbot" is about 40% higher than "Unknown" had been up to that point. I know
% that at one point Orbot started self-reporting its proxy type as "iptproxy"
% and the broker started recognizing it, but I can't get the timeline to make
% sense. It seems like the first version of Orbot to report itself as
% "iptproxy" was *after* the change in the graph.
% * 2022-03-21 Broker begins to recognize "iptproxy" as a probe type.
...
% * 2022-06-21 "unknown" disappears and "iptproxy" appears in descriptors.
...
Confusion with browser extension---do I install it if I \emph{am} censored,
or if I want to \emph{help} someone who is censored.
Evident in the number of Snowflake proxies in Iran.
...
\todo{Do some analysis using from the \href{https://metrics.torproject.org/rs.html\#toprelays}{toprelays}.}
