...
namely a Session Description Protocol (SDP) \emph{offer}~\cite{rfc8839}.
...
\todo{Do we really mean ``incoming connections,'' or ``incoming packets''?}
are allowed only if
...
The full stack of protocol layers is as shown below.
...
DTLS
key exchange
DTLS fingerprint % https://www.rfc-editor.org/rfc/rfc8122.html#section-5
...
% In fact, we prototyped successfully with QUIC.
...
is not any more fatal to a Snowflake session
than a dropped packet is to a TCP connection.

% we tried it without TT, was not fun

Snowflake's Turbo Tunnel layer
is partially redundant with the data channel SCTP layer
% reliable stream with multiplexing, same as KCP/smux
% (but we don't really use multiplexing)
...
- Broker rendezvous (e.g.\ TLS fingerprint \url{https://bugs.torproject.org/tpo/anti-censorship/pluggable-transports/snowflake/40207\#note_2849437} \url{https://github.com/net4people/bbs/issues/131}
