...
%   <date>     <chr>      <dbl>
% 1 2023-07-26 snowflake 68129
% 2 2023-07-27 snowflake 66794.
% 3 2023-07-28 snowflake 66177.
% 4 2023-07-29 snowflake 67472.
% 5 2023-07-30 snowflake 68530.
% 6 2023-07-31 snowflake 69167.
...
% 1 2023-07-26 31.42 TB 31.75 TB 40.30 GB      639.89 GB      31.38 TB  31.11 TB   31.24 TB
% 2 2023-07-27 30.87 TB 31.00 TB 19.36 GB      421.73 GB      30.85 TB  30.58 TB   30.71 TB
% 3 2023-07-28 30.77 TB 30.92 TB 18.67 GB      424.53 GB      30.75 TB  30.49 TB   30.62 TB
% 4 2023-07-29 31.20 TB 31.34 TB 18.61 GB      419.70 GB      31.18 TB  30.92 TB   31.05 TB
% 5 2023-07-30 30.66 TB 30.82 TB 18.71 GB      431.21 GB      30.64 TB  30.39 TB   30.52 TB
% 6 2023-07-31 31.49 TB 31.64 TB 18.68 GB      424.23 GB      31.47 TB  31.22 TB   31.34 TB
...
By~July, the second bridge supported about 18\% of Snowflake users.\todo{Revisit this when Orbot~17 hits the \href{https://play.google.com/store/apps/details?id=org.torproject.android}{Play Store}}
% > library("tidyverse")
% > WANTED_FINGERPRINTS <- c(
%     "7659DA0F96B156C322FBFF3ACCC9B9DC01C27C73" = "snowman",
%     "5481936581E23D2D178105D44DB6915AB06BFB7F" = "snowflake-01",
...
As~of \mbox{2023-07-31},
Snowflake had transferred 8.1~PB of circumvention data.
% > library("tidyverse")
% > WANTED_FINGERPRINTS <- c(
%     "7659DA0F96B156C322FBFF3ACCC9B9DC01C27C73" = "snowman",
%     "5481936581E23D2D178105D44DB6915AB06BFB7F" = "snowflake-01",
...
(29\% of bridge users) used Snowflake to connect to Tor.
...
% > library("tidyverse")
% > DATE_RANGE <- as.Date(c("2023-07-25", "2023-08-01"))
% > userstats <- bind_rows(
...
%   <chr>  <chr>            <dbl>   <dbl>
% 1 bridge non-snowflake  1359050    4.17
% 2 bridge snowflake       562232    1.72
% 3 relay  NA            30702598   94.1
% > userstats %>% filter(mode == "bridge") %>% mutate(percent = 100 * users / sum(users))
% # A tibble: 2 × 4
%   mode   transport       users percent
%   <chr>  <chr>           <dbl>   <dbl>
% 1 bridge non-snowflake 1359050    70.7
% 2 bridge snowflake      562232    29.3
...
% > library("tidyverse")
% > read_csv("figures/proxies/proxy-type.csv", col_types = cols()) %>% filter("2023-07-25" <= date & date < "2023-08-01") %>% group_by(type) %>% summarize(unique_ips = sum(unique_ips)) %>% mutate(percent = 100 * unique_ips / sum(unique_ips)) %>% ungroup()
% # A tibble: 4 × 3
%   type       unique_ips percent
%   <chr>           <dbl>   <dbl>
% 1 badge           7128.   0.772
% 2 iptproxy      134602.  14.6
% 3 standalone     33446.   3.62
% 4 webext        748009.  81.0
For comparison, there were about 2,000
of the more traditional style of Tor bridge at this time.
% https://metrics.torproject.org/networksize.html?start=2023-07-01&end=2023-07-31
% date,relays,bridges
% 2023-07-29,7154,2027
% 2023-07-30,7155,2017
% 2023-07-31,7141,1994
...
In~\autoref{sec:deployment} we saw how Snowflake's user counts
...
but as you can see in \autoref{fig:user-counts-ru},
...
See \autoref{fig:user-counts-ir}.
...
does not show any sudden increases or decreases
like others we have discussed so far.
There are a modest but respectable number of Snowflake users in China.
Despite the absence of any singular, sustained events,
we~have seen some evidence for short-term or tentative
...
recent enough to be in the scope of \autoref{fig:user-counts-cn}.
...
It is possible that Snowflake was not the main target for this change in 
blocking behaviour and was merely collateral damage. If~this interference 
indeed had to do with Snowflake,
...
a~country subject to even more extreme censorship
than the others we have seen.
Still, it~has happened at least twice
that the number of users dropped suddenly to zero,
as~you can see in \autoref{fig:user-counts-tm}.
Investigating the cause of blocks is particularly difficult in Turkmenistan.
The fraction of the population with access to the Internet is relatively small,
so~it is hard to find volunteer testers.
Even then, a~lack of reliable circumvention in general
hinders coordination and lengthens testing cycles.
We~were able to mitigate the blocking of Snowflake in Turkmenistan,
but only partly, and after protracted effort.

The first of these occasions was on \mbox{2021-10-24}.
Our investigation discovered multiple layered causes:
the default front domain used in rendezvous was blocked
by DNS and TLS filtering,
and certain UDP port numbers commonly used by STUN were blocked.
We~were able to mitigate the blocking
by providing alternative domains for rendezvous
and finding public STUN servers running on non-standard ports;
but not for all users in all ISPs.
% There had been about 750 relay users at the time.
...
Our investigation into the drop in users
took advantage of some peculiarities of the Turkmenistan firewall.
We~will state the essential information here;
...
TLS handshakes with a filtered SNI receive an injected RST packet
that tears down the TCP connection.
...
injection in Turkmenistan is bidirectional:
...
we~found that the default front domain used to access the broker
was blocked by injection at both the DNS and TLS layers---and
...
also that some alternative domains were not blocked.
...
But this on its own was not sufficient.
Though they could reach the broker,
...
Further testing revealed blocking of the default port number
of STUN, UDP port~3478.
...
cannot find its own ICE candidate addresses
(\autoref{sec:ice}),
which causes most WebRTC connections to proxies to fail.
(The exceptions are proxy that have no NAT or ingress filtering.
While there are some such proxies,
censorship in Turkmenistan also outright blocks
a large fraction of the IP address space,
including data center and VPS ranges where such proxies tend to run.)
...
As~chance would have it, the NAT discovery feature we need
...
commonly port 3479.
Changing STUN server addresses to those alternate port numbers
...
\todo{Investigate and discuss further blocking on \mbox{2022-08-03}.}
