...
it is diffuse.
In fact, if Snowflake has a single distinguishing characteristic,
it is that it pushes the idea of distributed, disposable
proxies further than has been done before,
by making proxies extremely cheap to run,
in web browsers, using WebRTC to communicate.

WebRTC is\ldots\ 
interacting suite of multiple protocols\ldots\ 
example applications\ldots\ 
importantly: built into browsers\ldots\ 

As is typical in circumvention research,
we assume a threat model where\ldots
