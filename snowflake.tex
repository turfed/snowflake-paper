...
\todo[inline]{Update this figure to show that unrestricted and restricted are different for proxies and clients.}
...
of restricted proxies.\todo{Does this mean 80\% of restricted proxies,
...
In~order for the broker to take NAT compatibility into account when matching,
clients and proxies need to self-assess their NAT type.
...
Proxies cannot use the same technique to discover their NAT type,
because the necessary STUN features are not available
to JavaScript code running in web browsers.
...
\autoref{fig:proxy-nat-type} shows the distribution of proxies by NAT type.
\todo{Consider moving \autoref{fig:proxy-nat-type} to \autoref{sec:experience} so as not to slow the pace here.}
