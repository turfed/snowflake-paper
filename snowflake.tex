...
The proxy transfers data back and forth
between the client and bridge
until the client terminates the session,
or the proxy itself stops operating
(when the web browser it is running in closes, for example).

\subsection{Data transfer}
\label{sec:data-transfer}

No~complicated or stateful processing is done by the proxy.
...
copying chunks of data in both directions without interpretation.
The proxy does not know what protocol it is conveying;
neither does it know whether it is the first
in a client's circumvention session
or a replacement for an earlier proxy.
Any features that require more state or awareness
must be provided not by proxies,
but by end-to-end coordination
between clients and the bridge.

The full protocol stack is shown below.
We will describe each layer and its purpose.
...
smux \\
\multicolumn{3}{@{}l}{Tor protocol (TLS)} \\
...
The UDP/\allowbreak DTLS/\allowbreak SCTP combination comes as a packaged unit
...
The large number of proxies presumed by the Snowflake model
precludes checking each one for trustworthiness.
If~a suitable encrypted and authenticated protocol is used,
as with the Tor protocol in our deployment,
then even a malicious proxy cannot read or subvert
information transferred between client and bridge.
It~may be considered an instance of the
``untrusted messenger'' model of
Feamster et~al.~\cite[\S 3]{Feamster2003a}:
what they call a ``messenger'' is our temporary proxy;
...
\todo[inline]{Make the size of the horizontal axis equal in each graph (they vary because of different numbers of digits).}
\end{figure}
