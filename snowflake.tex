...
but as a bridge fingeprint (a~\mbox{40-digit} hexadecimal string).
The broker maps the fingerprint to a WebSocket URL
...
Proxies check the hostname of the URL's
...
that was partially % "partially" because only in Tor Browser, not Orbot at that time
mitigated only in February~2023.
}
...
The \mbox{supported\_groups} distinguisher had already been
...
% On 2022-05-16, an anonymous reporter said that specific contents of supported_groups,
% this time in Client Hello, was being used as a distinguisher.
...
% ValdikSS confirmed the blocked byte pattern, though also said that Snowflake worked despite it:
% https://bugs.torproject.org/tpo/anti-censorship/censorship-analysis/40030#note_2804998
% The fact that it worked may have to do with the fact that the DTLS client role is not always played
% by a Pion implementation.
% We made a test release with a modified supported_groups in Client Hello:
...
% https://archive.org/details/tor-browser-11.5a13-snowflake-dtlslib-20220712-9d73998bca39
% Reports on NTC were that the supported_groups change did not unblock anyone who had been blocked.
% On 2022-06-21, another anonymous user reported that supported_groups in Client Hello
% had ceased to be a distinguisher, and the censor was now using Hello Verify Request instead.
...
% In summary, the reported supported_groups in Client Hello distinguisher doesn't appear to have had
% a large effect on user counts, and may have gone away without our having to do anything.
% But who knows, maybe it was the cause of the sharp decline in early April 2022?

Second round of DTLS fingerprinting, this time targeting DTLS Hello Verify Request?
Initially not noticed, lost in the shuffle with Iran?
...
\url{https://ntc.party/t/in-case-snowflake-rendezvous-gets-blocked/1857/9}
\url{https://gitlab.torproject.org/tpo/anti-censorship/pluggable-transports/snowflake/-/merge_requests/131}
\url{https://gitlab.torproject.org/tpo/applications/tor-browser-build/-/merge_requests/637}
Snowflake 2.5.1, Tor Browser 12.0.3
...
As~with the supported\_groups distinguisher,
the Hello Verify Request distinguisher
would not have been effective when the DTLS server
was a web browser proxy.
% "Snowflake will only work if you have a browser version of the proxy and the snowflake client is a DTLS Client." https://bugs.torproject.org/tpo/anti-censorship/censorship-analysis/40030#note_2823140

(The increase in circumventing users was,
however, more than counterbalanced by a loss of users
accessing Tor directly.)
