...
As~of October 2023, Snowflake supports an estimated average 35,000 concurrent users
% > library("tidyverse")
...
Protocol fingerprinting, rather than proxy address discovery,
...
Xie et~al.~\cite{Xie2023a} trained a decision tree using packet
size, directionality, and timing features with the aim of
distinguishing Snowflake rendezvous from web browsing
traffic. They acknowledge that if the aim is to block the
rendezvous connection before it completes, detection must occur
as early as possible in the flow and therefore only consider the
first 30 packets. The lowest reported false positive rate of their
experiments is 0.25\%, however the low base rate of occurrence of Snowflake
connections makes us believe that the collateral damage of this technique
would be prohibitively high.

\todo{Cite ``On precisely detecting\ldots''~\cite{Wails2024a}.}
...
The next event that had a major effect on Snowflake usage
was nationwide protests in Iran that started on \mbox{2022-09-16}.
The government imposed periodic network shutdowns
and additional network blocking,
which were severe even by the standards of a country already notorious
...
At~that time, about 0.8\% of all Tor users\todo{
As of \mbox{2023-10-15}, relay users are currently inflated due to
...
was simple carelessness on our part.
