...
% These labels were used in RFC 3489 (2003; back when STUN was "Simple Traversal of UDP through NAT").
% https://datatracker.ietf.org/doc/html/rfc3489#section-5
% They aren't used in the obsoleting RFC 5389 and RFC 8489 (after it became "Session Traversal Utilities for NAT").
\begin{table}
...
(no~fallback to TURN).
Variations are labelled as (\(\mathit{mapping}\),~\(\mathit{filtering}\)) behavior pairs.
The possible behaviors are: Endpoint-Independent (EI), Address-Dependent (AO),
and Address- and Port-Dependent (AP). We introduce the shorthand Endpoint-Dependent (ED)
to indicate either AO or AP behavior.
The incompatible cases,
which the broker tries to avoid when matching,
are when one peer's NAT has ED mapping
and the other's has ED mapping or
AP filtering.
...
shows the pairwise compatibility of possible NAT variations~\cite{rfc5780}.
Endpoint-dependent NAT mapping behaviour\todo{Cite RFC for these.}
(be~it address-dependent only, or both address- and port-dependent),
indicates that the address a local endpoint is translated to depends on
the remote endpoint's address (or both address and port). Endpoint-dependent NAT
filtering indicates that incoming packets from a remote endpoint are
allowed only if outgoing packets have already been sent to that remote.
As~the incompatible cases always involve endpoint-dependent NAT mapping
(sometimes referred to as symmetric NAT),
...
(24\%~of bridge users) used Snowflake.
% > library("tidyverse")
% > WANTED_FINGERPRINTS <- c(
%     "7659DA0F96B156C322FBFF3ACCC9B9DC01C27C73" = "snowman",
%     "5481936581E23D2D178105D44DB6915AB06BFB7F" = "snowflake-01",
...
The technique was to record the set of proxy IP addresses
...
we~merged sketches over the prior 24 hours
to serve as a reference.
We~similarly merged other 24-hour windows,
offset by \(+1, +2, \ldots, +40\) hours,
and computed the size of their intersection with the reference.
...
For reasons of scalability and performance, though,
...
both the impact of and possible defenses to such attacks.
...
\paragraph{What do alternative Snowflake deployments look like?}

A~natural extension of Snowflake would be
to have it access systems other than Tor, such as ordinary
VPNs.
...
which avoids the need for an exit policy at the proxies.
However, Tor has relatively low speed and lacks
support for UDP and other non-TCP protocols.
One consideration for a world with multiple Snowflake deployments
is the management of the pool of proxies.
...
Another challenge of a~shared deployment is the increase in
development friction (e.g., the difficulty of altering the proxy protocol).
...
makes possible the multiplexing of client traffic
