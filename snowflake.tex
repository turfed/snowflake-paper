...
client and proxy arrange a connection out of band,
and the proxy persists for the full duration of a circumvention session.
Protozoa's specialty is resistance to traffic analysis:
using an authentic WebRTC audio or video stream as a carrier,
it replaces the encrypted portions of encoded media frames
with its own covert ciphertexts of identical size,
such that the circumvention tunnel retains the traffic features
of the original carrier stream.
...
%     group_by(date) %>%
%     summarize(across(c(read, write, `dirreq-read`, `dirreq-write`), sum, na.rm = TRUE, .names = "cumul_{.col}")) %>%
%     mutate(
...
% 1 2023-05-26 37.88 TB   37.95 TB    17.61 GB            359.12 GB            37.87 TB        37.59 TB         37.73 TB
% 2 2023-05-27 37.21 TB   37.29 TB    17.54 GB            380.37 GB            37.19 TB        36.91 TB         37.05 TB
% 3 2023-05-28 35.87 TB   35.97 TB    17.36 GB            384.49 GB            35.86 TB        35.59 TB         35.72 TB
% 4 2023-05-29 36.14 TB   36.25 TB    19.26 GB            416.83 GB            36.12 TB        35.84 TB         35.98 TB
% 5 2023-05-30 36.41 TB   36.52 TB    18.25 GB            418.39 GB            36.39 TB        36.10 TB         36.25 TB
% 6 2023-05-31 36.30 TB   36.40 TB    17.01 GB            400.02 GB            36.28 TB        36.00 TB         36.14 TB
...
The broker is responsible for matching clients with proxies and facilitating the
exchange of information necessary for both peers to establish a WebRTC connection.
...
\todo{Are there other matching criteria? \#Comment: Currently no: \url{https://gitlab.torproject.org/tpo/anti-censorship/pluggable-transports/snowflake/-/blob/main/broker/ipc.go\#L235} }
...
\todo{This may not be fully accurate, see comments in source
%
% as it is possible to require non-user-specific secret to get access to the channel,
% and/or use user-specific credenital to authenticate to the channel.
% Let's say in the case of https://shadowsocks.org/doc/sip008.html
% A general purpose client don't ship with subscrition links in the form of HTTPS links
% like https://s3.aws/secretbucket/usernameandpassword
% A client would require user to input this link and client will in turn fetch configuration from such link
% An attacker is prevented from getting the path part of url,
% even with privilaged network position
% and won't be able to probe if s3.aws hosts such an rendezvous channel without secret(path)
% this is needed as not all websites have high collateral damages,
% and the one providing the configuration hope that censor have no idea which server name is used
...
it is conservatively assumed to be ``restricted'' \todo{maybe here should be "unknown"?} 
and the broker distributes it along with proxies with known restricted NATs. 
...
%  1 2023-02-20 snowflake 79040.            12     12   snowflake-01   95.9
%  2 2023-02-20 snowflake  3407.            12     12   snowflake-02    4.13
%  3 2023-02-21 snowflake 76365.            12     12   snowflake-01   95.8
%  4 2023-02-21 snowflake  3354.            12     12   snowflake-02    4.21
%  5 2023-02-22 snowflake 72064.            12     12   snowflake-01   95.6
%  6 2023-02-22 snowflake  3356.            12     12   snowflake-02    4.45
%  7 2023-02-23 snowflake 71508.            12     11.1 snowflake-01   95.5
%  8 2023-02-23 snowflake  3408.            12     12   snowflake-02    4.55
%  9 2023-02-24 snowflake 71549.            12     11.9 snowflake-01   95.4
% 10 2023-02-24 snowflake  3444.            12     12   snowflake-02    4.59
% 11 2023-02-25 snowflake 70069.            12     12   snowflake-01   95.2
% 12 2023-02-25 snowflake  3506.            12     12   snowflake-02    4.77
% 13 2023-02-26 snowflake 70590.            12     12   snowflake-01   95.3
% 14 2023-02-26 snowflake  3506.            12     12   snowflake-02    4.73
% 15 2023-02-27 snowflake 69135.            12     12   snowflake-01   95.2
% 16 2023-02-27 snowflake  3454.            12     12   snowflake-02    4.76
...
%   date       cumul_read cumul_write `cumul_dirreq-read` `cumul_dirreq-write` cumul_good_read cumul_good_write cumul_good_avg
%   <date>     <chr>      <chr>       <chr>               <chr>                <chr>           <chr>            <chr>
% 1 2023-04-30 5.02 PB    5.05 PB     5.51 TB             94.14 TB             5.01 PB         4.96 PB          4.98 PB
...
representing about 88\%\todo{Update percentage before submission.} of daily IP addresses.
...
% Problems with antimalware
% "Purchase new snowflake domain to fix 'safe browsing' issue" https://bugs.torproject.org/tpo/anti-censorship/pluggable-transports/snowflake/31250
% "Make *.freehaven.net domains be CNAMEs for *.torproject.net, not *.bamsoftware.com" https://bugs.torproject.org/tpo/anti-censorship/pluggable-transports/snowflake/40028
% "Avast blocking connection to 02.snowflake.torproject.net on Chrome" https://bugs.torproject.org/tpo/anti-censorship/pluggable-transports/snowflake-webext/85
...
Althought clients without STUN candidates could theoretically connect
