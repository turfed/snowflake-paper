...
\maketitle
\todo{Change the title: ``peer-to-peer'' is misleading as proxies are not themselves clients.}
...
Flash proxy~\cite{Fifield2012a} similarly used a model
...
Flash proxy was deployed to users in Tor Browser
...
it was decided to begin Snowflake as an independent project,
...
% https://docs.google.com/document/d/1t_30vX7RcrEGuWwcg0Jub-HiNI0Ko3kBOyqXgrQN3Kw/edit#heading=h.xapcidyka1bk
% (from https://github.com/UWNetworksLab/uProxy-p2p/tree/4edae62ec1110ea2b0b795fb68b95d3f5d233244#status)
privileged proxies, obfuscated WebRTC data channels
``uProxy depends on leveraging existing trust relationships to to find and use a proxy''
significant work required in Snowflake, relative to uProxy, for WebRTC outside the browser

% Aside from flash proxy\index{flash proxy},
% the most similar existing design was a former version of uProxy\index{uProxy}~\cite{uproxy}
% (an upcoming revision will work differently).
...
% using WebRTC\index{WebRTC}; the proxy would then directly fetch
% the client's requested URLs\index{URL}.
% Snowflake centralizes the proxy discovery process\index{proxy distribution},
...
many modes: direct access for uncensored destinations, CacheBrowsing when possible, buddies as a last resort
standalone Buddy software, not in a browser
clients can also act as Buddies
option to proxy directly from a Buddy, no then through an additional proxy hop, Buddies need to specify allowed content categories
takes the principled approach of prioritizing circumvention over anonymity and privacy
...
\todo{Cite Untrusted Messenger Discovery~\cite[\S 3]{Feamster2003a} here or in \autoref{sec:data-transfer}---decouples ``messenger'' and ``portal''.}
...
Snowflake clients and proxies all have one of a few possible
network configurations that can be categorized by two factors:
(1)~the mapping behaviour of
the peer's network address translation (NAT) tables; and
(2)~the filtering behaviour of the peer's firewalls.
Table~\ref{tab:nat-matching}
shows which NAT variations are compatible with each other.
...
\begin{description}
    ...
    \item[Port-restricted cone] which acts the same as a restricted-cone
        NAT except that incoming connections are filtered if there was not a previous
        outgoing connection to the remote IP and port pair.
    \item[Symmetric] where connections from an
        internal IP and port pair are mapped to an external IP--port pair depending
...
\todo[inline]{The table is symmetric; show only half of it.
Fix centering with \texttt{\textbackslash rlap} slanted labels.}
\end{table}
...
Instead, we use a technique similar to that employed by MassBrowser~\cite[\S V-A]{Nasr2020a}
...
\todo{Media channel also would not use DTLS.}
...
\todo{Mention traffic shaping?}
% The Snowflake protocol supports traffic shaping (inside the WebRTC layer),
% but it is currently disabled.
% As \autoref{sec:experience} shows,
% censors are still going after easier targets.
% Circumvention research awaits a good treatment of
% exactly what is a good strategy for traffic shaping to
% best mitigate attacks of this nature,
% and an analysis of how well such attacks work outside the laboratory.
