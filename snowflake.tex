...
can we say that blocking action
clearly targeted Snowflake specifically;
in~the other cases the evident blocking rules were broad enough
that they may have affected Snowflake only incidentally.
...
Still, there have been at least two times
when the number of users dropped suddenly to zero
and took months to recover,
as~you can see in \autoref{fig:user-counts-tm}.
Whether these events were targeted attacks on Snowflake is hard to say,
because censorship in Turkmenistan is particularly
heavy-handed---both in the sense that it is excessively harsh
and in the sense that it is a blunt weapon wielded without precision.

On~\mbox{2021-10-24} the number dropped suddenly to zero,
