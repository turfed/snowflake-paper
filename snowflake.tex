...
% https://www.usenix.org/conference/usenixsecurity24/instructions-presenters
...
As~of March 2024, Snowflake supports an estimated 35,000 average concurrent users
% > library("tidyverse")
...
at an average total transfer rate of 2.7~Gbit/s,
...
shown abstractly in \autoref{fig:rendezvous}.
...
would become targets of blocking.
...
most applications want to connect \emph{a particular} pair of peers,
...
% https://gitlab.torproject.org/tpo/anti-censorship/pluggable-transports/snowflake/-/wikis/NAT-matching?version_id=dd1f328e2c9570eb252c43a053bdec0314c73345#fn-1-2223
...
The broker counts the number of unmatched (\(U\))
and matched (\(M\)) client rendezvous requests per day.
...
% At the same time: https://bugs.torproject.org/tpo/anti-censorship/pluggable-transports/snowflake/40228
...
and the client's application streams.\urlfootnote{
...
and therefore shares the fingerprinting concerns of TLS~\cite{Frolov2019a}.
...
but found them still to be prohibitively imprecise at realistic base rates.
...
For example, when the graph passes through 10,000 on \mbox{2022-02-04},
...
% "First working bundles with Snowflake, for linux only" https://bugs.torproject.org/tpo/anti-censorship/pluggable-transports/snowflake/19001#note_2591210
}
and for macOS
...
% "mac reproducible build" https://bugs.torproject.org/tpo/anti-censorship/pluggable-transports/snowflake/19001#note_2591242
...
until we fixed the bug in mid-March 2023.\urlfootnote{
...
before the performance bug was fixed.)
% 2024-05-13 Cecylia has also demonstrated the effect of the bug in network simulation experiments.
% https://forum.torproject.org/t/experimenting-with-tor-pluggable-transports-in-shadow/9117/2#snowflake-experimental-results-3
...
and Orbot~17 on \mbox{2024-02-09}.
...
Something similar happened on \mbox{2024-03-01},
when the CDN we had been using in the default configuration
of domain fronting rendezvous
stopped supporting domain fronting.\urlfootnote{
...
but \autoref{fig:client-counts} shows an immediate decline in users and bandwidth
of about~30\%.
...
%     "91DA221A149007D0FD9E5515F5786C3DD07E4BB0" = "snowflake-02"
%   )
% > DATE_RANGE <- as.Date(c("2024-02-23", "2024-03-01"))
% > userstats <- bind_rows(
...
% > library("tidyverse")
% > proxy_type <- read_csv("figures/proxies/proxy-type.csv", col_types = cols()) %>% filter("2024-02-01" <= date & date < "2024-03-01")
...
% https://metrics.torproject.org/networksize.csv?start=2024-02-01&end=2024-02-29
% date,relays,bridges
...
in late 2022 (particularly after the events in Iran of \autoref{sec:clients}),
...
and the timing of known political events.\urlfootnote{
...
It~was DTLS fingerprinting\urlfootnote{
...
The extension's presence in Server Hello was in fact a bug\urlfootnote{
...
The presence of a \mbox{supported\_groups} extension in Client Hello is not unusual,
...
looked for the server sending a Hello Verify Request message.\urlfootnote{
...
this time containing a copy of the cookie~\cite[\S 5.1]{rfc9147}.
...
The first reports of this blocking rule arrived in July~2022;
...
depending on hardware capabilities and how it is compiled.\urlfootnote{
...
in the Snowflake client using uTLS---but
...
(\autoref{sec:multi-bridge}), though such a
...
an~effect that is visible in \autoref{fig:client-counts-cn}.
On~\mbox{2023-05-15}, the blocking went away and the user count returned to what it had been.
...
(a~possible counterexample to our claims about traffic analysis in \autoref{sec:fingerprinting}).
...
after examining data from Censored Planet~\cite{Raman2020c},
a~censorship measurement platform that, at~that time,
had continuous measurements
of the domain from one autonomous system in Turkmenistan.
...
and is what precipitated the drop seen on that date in \autoref{fig:client-counts-tm}.
