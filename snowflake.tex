...
for technical reasons having to do with
integration with Tor.
Here we will discuss design considerations
and what tradeoffs are involved in
the design we settled~on.

What is called the bridge is actually a pipeline of several components.
One component terminates WebSocket connections incoming from proxies,
one decodes the contained Turbo Tunnel packets and reassembles them into streams,
and one forwards the streams to their eventual destination
(in~our deployment, this last component is Tor).
...
A~deployment not based on Tor would have more flexibility.
