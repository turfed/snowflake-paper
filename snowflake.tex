...
they use different cryptographic containers,
and an unprivileged observer can tell which is which.
...
we must specify some concrete proxy protocol
that the client can use to tell the bridge
...
by curious or malicious proxies.
...
Tor has its own mechanisms for conveying destination addresses
to an eventual exit node,
and on the client side presents a standard SOCKS interface.
...
Tor also has drawbacks and complications,
which we will comment on in
\autoref{sec:multi-bridge}
and
\autoref{sec:future}.

Snowflake's decoupling of the server
that accesses remote destinations
from the means of accessing it
may be seen as an instance of the
...
It~is the presence of an end-to-end secure protocol
between client and bridge that makes this decoupling possible.
Clients have confidence to send their traffic through
untrusted intermediaries when they know those intermediaries
can see only ciphertext.
Running a proxy is worry-free,
as proxy operators know client traffic will not be
mistakenly attributed to them.
A~Snowflake connection requires less trust by all concerned
than most other forms of circumvention proxy.
...
% "AVG blocked connection of Chrome+Snowflake due to 'URL:Blacklist' threat" https://bugs.torproject.org/tpo/anti-censorship/pluggable-transports/snowflake/40274
