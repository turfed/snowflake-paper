...
%     group_by(date, transport) %>% mutate(percent = mean / sum(mean) * 100) %>% filter(country == "ir") %>% ungroup()
% # A tibble: 7 x 7
%   date       transport country    low   high   mean percent
%   <date>     <chr>     <chr>    <dbl>  <dbl>  <dbl>   <dbl>
...
Snowflake had transferred 4.06~PB of user data.\todo{Estimate and subtract non-Snowflake (i.e.~$\langle\mathrm{OR}\rangle$) bytes, even though likely negligible.}
% > library("tidyverse")
% > options(width = 200)
% > WANTED_FINGERPRINTS <- c(
%     "7659DA0F96B156C322FBFF3ACCC9B9DC01C27C73" = "snowman",
...
% 1 2023-03-31 4.09 PB    4.12 PB     4.97 TB             83.47 TB             4.09 PB         4.03 PB          4.06 PB
...
% > library("tidyverse")
% > DATE_RANGE <- as.Date(c("2023-03-23", "2023-03-30"))
% > userstats <- bind_rows(
%     read_csv("figures/users/userstats-relay-country-2023-01-04-all-2023-04-04-off.csv", comment = "#") %>%
%       mutate(mode = "relay"),
%     read_csv("figures/users/userstats-bridge-transport-2023-01-04-2023-04-04-! OR -snowflake- OR .csv", comment = "#") %>%
%       mutate(mode = "bridge")
%   ) %>%
...
% * <chr>  <chr>            <dbl>   <dbl>
% 1 bridge non-snowflake  1374662    6.01
% 2 bridge snowflake       607905    2.66
% 3 relay  NA            20872771   91.3
% > userstats %>% filter(mode == "bridge") %>% mutate(percent = 100 * users / sum(users))
% # A tibble: 2 x 4
%   mode   transport       users percent
%   <chr>  <chr>           <dbl>   <dbl>
% 1 bridge non-snowflake 1374662    69.3
% 2 bridge snowflake      607905    30.7
...
\todo{Reconsider the section title:
``Performance'' is unspecific given what the section now contains.}

Snowflake is positioned in the class
of low-latency, relatively high-bandwidth circumvention systems.
Even coupled with Tor,
as long as you get a fast enough proxy,
the connection should be suitable for
interactive web browsing and video streaming.
We~have remarked, in \autoref{sec:deployment},
on some of the growing pains
experienced in having performance keep pace with demand.
Here we comment on a further design
intended to enable increased performance:
distributing traffic over more than one bridge server.
...
As~of this writing, there are two backend bridges.
There are two reasons why distributing load
over more than one bridges is less than trivial,
one related to Snowflake itself
and one having to do with its interface with Tor.
...
limitations not in Snowflake itself,
but in the way it interacts with the Tor client.
a deployment not built on Tor would have more flexibility here

loosely coupled
frontend server interface, terminates WebSocket, manages Turbo Tunnel session state
recall from \autoref{sec:data-transfer} that a Snowflake session has long-term end-to-end state maintained by the client and the bridge
backend proxy interface (Tor)
easily separable
Turbo Tunnel state not so easily separable

just as you cannot expect to resume an in-progress TCP connection
with a different endpoint, unless the endpoints are
coordinating behind the scenes

client has some control over what the proxy connects to,
safeguards against misuse
% https://forum.torproject.net/t/anyone-experiencing-problems-with-snowflake-proxy/6938/15

However, the design of snowflake requires server to keep state for the clients across connections that prevents server side load balancing. To enable the use of more than one snowflake server, a new design amendment is necessary.
...
\todo{Do some analysis from the \href{https://metrics.torproject.org/rs.html\#toprelays}{toprelays}.}
